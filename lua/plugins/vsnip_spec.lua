return {
	...
	},
	{
		...
		init = function()
			...
			map_expr({"i", "s"}, EXPAND, function()
				return vim.fn["vsnip#expandable"]() == 1 and "<Plug>(vsnip-expand)" or "<C-j>"
			end)

			-- vsnip#expand-or-jump
			map_expr({ "i", "s" }, EXPAND_OR_JUMP, function()
				return vim.fn  == 1 and "<Plug>(vsnip-expand-or-jump)" or "<C-l>"
			end)

			-- vsnip#jump-next
			map_expr({ "i", "s" }, JUMP_NEXT, function()
				return vim.fn  == 1 and "<Plug>(vsnip-jump-next)" or "<Tab>"
			end)

			-- vsnip#jump-prev
			map_expr({ "i", "s" }, JUMP_PREV, function()
				return vim.fn["vsnip#jumpable"](-1) == 1 and "<Plug>(vsnip-jump-prev)" or "<S-Tab>"
			end)

			-- Non-expression mappings (no need for expr=true)
			vim.keymap.set("n", SNIP_SELECT, "<Plug>(vsnip-select-text)")
			vim.keymap.set("x", SNIP_SELECT, "<Plug>(vsnip-select-text)")
			vim.keymap.set("n", SNIP_CUT, "<Plug>(vsnip-cut-text)")
			vim.keymap.set("x", SNIP_CUT, "<Plug>(vsnip-cut-text)")
		end,
	},
}
